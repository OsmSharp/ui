...
namespace Tools.Math.TravellingSalesman.LocalSearch.HillClimbing3Opt
{
    ...
    public class HillClimbing3OptSolver : ISolver
    {
        ...
        public IRoute Solve(IProblem problem)
        {
            ...
            while (improvement)
            {
                ...
                foreach (int v_1 in route)
                {
                    ...
                    {
                        if (this.Try3OptMoves(problem, route, v_1))
                        {
                            improvement = true;
                            break;
                        }
        ...
        /// <returns></returns>
        public bool Try3OptMoves(IProblem problem, IRoute route, int v_1)
        {
            // get v_2.
            int v_2 = route.GetNeigbours(v_1)[0];
            BetweenEnumerable between_v_2_v_1 = new BetweenEnumerable(
                route, v_2, v_1);
            int v_3 = -1;
            foreach (int v_4 in between_v_2_v_1)
            {
                if (v_3 >= 0 && v_3 != v_1)
                {
                    ...
                    {
                        if (!problem.Get10NearestNeighbours(v_1).Contains(v_4))
                        {
                            break;
                        }
                    }
                    if (this.Try3OptMoves(problem, route, v_1, v_2, v_3, v_4))
                    {
                        return true;
                    }
                }
        ...
        /// <param name="v_1"></param>
        /// <returns></returns>
        public bool Try3OptMoves(IProblem problem, IRoute route, int v_1, int v_2, int v_3)
        {
            // get v_4.
            int v_4 = route.GetNeigbours(v_3)[0];
            return this.Try3OptMoves(problem, route, v_1, v_2, v_3, v_4);
        }
        ...
        /// <returns></returns>
        public bool Try3OptMoves(IProblem problem, IRoute route, int v_1, int v_2, int v_3, int v_4)
        {
            BetweenEnumerable between_v_4_v_1 = new BetweenEnumerable(route, v_4, v_1);
            foreach (int v_5 in between_v_4_v_1)
            {
                if (v_5 != v_1)
                {
                    if (this.Try3OptMove(problem, route, v_1, v_2, v_3, v_4, v_5))
                    {
                        return true;
                    }
                }
        ...
        /// <returns></returns>
        public bool Try3OptMove(IProblem problem, IRoute route, int v_1, int v_2, int v_3, int v_4, int v_5)
        {
            // get v_6.
            int v_6 = route.GetNeigbours(v_5)[0];
            return this.Try3OptMove(problem, route, v_1, v_2, v_3, v_4, v_5, v_6);
        }
        ...
        /// <returns></returns>
        public bool Try3OptMove(IProblem problem, IRoute route, int v_1, int v_2, int v_3, int v_4, int v_5, int v_6)
        {
            ...
            float weight = problem.WeightMatrix[v_1][v_2] +
                problem.WeightMatrix[v_3][v_4] +
                problem.WeightMatrix[v_5][v_6];

            // calculate the total weight of the 'new' arcs.
            float weight_new = problem.WeightMatrix[v_1][v_4] +
                problem.WeightMatrix[v_3][v_6] +
                problem.WeightMatrix[v_5][v_2];
