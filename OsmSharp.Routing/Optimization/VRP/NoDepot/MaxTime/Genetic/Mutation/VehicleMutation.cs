...
namespace OsmSharp.Routing.Optimization.VRP.NoDepot.MaxTime.Genetic.Mutation
{
    /// <summary>
        ...
        IMutationOperation<MaxTimeSolution, MaxTimeProblem, Fitness>
    {
            ...
            Individual<MaxTimeSolution, MaxTimeProblem, Fitness> mutating)
        {
            ...
            if (solution.Count > 1)
            {
                ...
                // 
                int source_route_idx = OsmSharp.Math.Random.StaticRandomGenerator.Get().Generate(solution.Count);
                ...
                if (source_size > avg)
                {
                    int size = OsmSharp.Math.Random.StaticRandomGenerator.Get().Generate(source_size / 2);
                    //int size = OsmSharp.Math.Random.StaticRandomGenerator.Get().Generate(source_size);
                    //int size = OsmSharp.Math.Random.StaticRandomGenerator.Get().Generate(source_size / 3);
                ...
                { // route is small; add it to a neighbour route.
                    ...
                    do
                    {
                        target_route_idx = OsmSharp.Math.Random.StaticRandomGenerator.Get().Generate(solution.Sizes.Count);
