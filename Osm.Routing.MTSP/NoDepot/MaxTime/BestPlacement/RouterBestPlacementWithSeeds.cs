...
namespace Osm.Routing.Core.VRP.NoDepot.MaxTime.BestPlacement
{
        ...
        where ResolvedType : ILocationObject
    {
        ...
        public RouterBestPlacementWithSeeds(IRouter<ResolvedType> router, Second max, Second delivery_time)
            ...
            Second max)
        {
            ...
            List<int> selectable_customers = new List<int>(customers);
            ...
            while (selectable_customers.Count > 0)
            {
                ...
                //int random_idx = Tools.Math.Random.StaticRandomGenerator.Get().Generate(selectable_customers.Count);
                //int customer = selectable_customers[random_idx];
                ...
                for (int route_idx = 0; route_idx < routes.Count; route_idx++)
                {
                    ...
                    if (weight + result.Increase + calculator.DeliveryTime < max.Value)
