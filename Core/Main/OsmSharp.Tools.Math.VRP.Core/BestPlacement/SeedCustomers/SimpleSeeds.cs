        ...
        public ICollection<int> SelectSeeds(IProblemWeights weights, int k)
        {
            ...
            while (tries < max_tries)
            {
                ...
                { // ok there is an improvement!
                    Tools.Core.Output.OutputStreamHost.WriteLine(string.Format("Seed new minimal: {0}->{1}",
                        minimal, new_minimal));
            ...
            Tools.Core.Output.OutputStreamHost.WriteLine(string.Format("Seed distance: {0}", 
                minimal));
