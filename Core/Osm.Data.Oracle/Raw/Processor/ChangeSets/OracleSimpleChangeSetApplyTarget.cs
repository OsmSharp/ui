...
namespace Osm.Data.Oracle.Raw.Processor.ChangeSets
{
    public class OracleSimpleChangeSetApplyTarget : DataProcessorChangeSetTarget
    {
        ...
        private void Modify(SimpleWay way)
        {
            ...
            // raise the modified event.
            this.RaiseChange(SimpleChangeType.Modify, SimpleOsmGeoType.Way, way.Id.Value);
        }
            ...
            while (reader.Read())
            {
                ...
                if (idx < nodes.Count)
                {
                    if (node_id != nodes[idx])
                    {
                        ...
                        // raise the modified event.
                        this.RaiseChange(SimpleChangeType.Modify, SimpleOsmGeoType.Node, node_id);
                        this.RaiseChange(SimpleChangeType.Modify, SimpleOsmGeoType.Node, nodes[idx]);
                    }
                }
                else
                {
        ...
        private void ModifyRelationMembers(long id, IList<SimpleRelationMember> members)
        {
            OracleCommand command;
        ...
        private void ModifyTags(long id, IDictionary<string, string> new_tags, string table, string ref_column)
        {
            OracleCommand command;

            Dictionary<string, string> tags_to_insert = null;
            if (new_tags == null)
            {
                tags_to_insert = new Dictionary<string, string>();
            }
            else
            {
                tags_to_insert = new Dictionary<string, string>(new_tags);
            }
            // suppose there are no tags present yet.
            Dictionary<string,string> tags_to_update = new Dictionary<string,string>();
            Dictionary<string,string> tags_to_delete = new Dictionary<string,string>();            

            // adjust the data based on the tags already present.
            command = this.CreateCommand(string.Format("select * from {0} where {1}=:{1}",table,ref_column));
            command.Parameters.Add(ref_column, id);
            ...
            while (reader.Read())
            {
                ...
                if (tags_to_insert.ContainsKey(k))
                {
                    // there is at least an update or no insert.
                    string new_value = tags_to_insert[k];
                    tags_to_insert.Remove(k);
            ...
            foreach(string key in tags_to_delete.Keys)
            {
                command = this.CreateCommand(string.Format("delete from {0} where {1}=:{1} and key=:key", table, ref_column));
            ...
            // update tags.
            foreach (KeyValuePair<string, string> pair in tags_to_update)
            {
                command = this.CreateCommand(string.Format("update {0} set value=:value where {1}=:{1} and key=:key", table, ref_column));
            ...
            // insert tags.
            foreach (KeyValuePair<string, string> pair in tags_to_insert)
            {
        ...
        private void Create(SimpleWay way)
        {
            OracleCommand command;
        ...
        private void CreateRelationMembers(long id, IList<SimpleRelationMember> members)
        {
            OracleCommand command;
        ...
        public void CreateTags(long id, IDictionary<string, string> new_tags, string table, string ref_column)
        {
            ...
            Dictionary<string, string> tags_to_insert;
            if (new_tags == null)
            {
                tags_to_insert = new Dictionary<string,string>();
            }
        ...
        private void Delete(SimpleWay way)
        {
            OracleCommand command;
