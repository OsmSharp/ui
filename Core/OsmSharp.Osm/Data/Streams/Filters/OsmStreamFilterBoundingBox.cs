...
namespace OsmSharp.Osm.Data.Streams.Filters
{
    ...
    public class OsmStreamFilterBoundingBox : OsmStreamFilter
    {
            ...
            { // just go over all nodes and ways.
                if (this.Reader.MoveNext())
                {
                    ...
                    if (!finished)
                    {
                        ...
                        {
                            SimpleOsmGeo current = this.Reader.Current();
        ...
        /// <returns></returns>
        private bool IsInBB(SimpleOsmGeo osmGeo)
        {
            bool isIn = false;
            switch (osmGeo.Type)
            {
                case SimpleOsmGeoType.Node:
                    isIn = _box.IsInside(new GeoCoordinate(
                        (osmGeo as SimpleNode).Latitude.Value,
                        (osmGeo as SimpleNode).Longitude.Value));
                    break;
                case SimpleOsmGeoType.Way:
                    foreach (long nodeId in (osmGeo as SimpleWay).Nodes)
                    {
                ...
                case SimpleOsmGeoType.Relation:
                    ...
                    {
                        foreach (SimpleRelationMember member in (osmGeo as SimpleRelation).Members)
                        {
