...
namespace OsmSharp.Osm.Data.Streams.Filters
{
    ...
    public class OsmStreamFilterProgress : OsmStreamReader
    {
        ...
        {
            SimpleOsmGeo current = _reader.Current();
            ...
            switch (current.Type)
            {
                case SimpleOsmGeoType.Node:
                    ...
                    if ((_node % 10000) == 0)
                    {
                        ...
                        OsmSharp.Output.OutputStreamHost.WriteLine("Node[{0}]: {1}nodes/s @ {2}MB",
                        ...
                        float seconds = ((float)(stop - _start)) / (float)TimeSpan.TicksPerSecond;
                        if ((_node % 1000000) == 0) { GC.Collect(); }
                        OsmSharp.Output.OutputStreamHost.WriteLine("Node[{0}]: {1}nodes/s",
                ...
                case SimpleOsmGeoType.Relation:
                    ...
                    if ((_relation % 1000) == 0)
                    {
                        ...
                        OsmSharp.Output.OutputStreamHost.WriteLine("Relation[{0}]: {1}relations/s @ {2}MB",
                        ...
                        float seconds = ((float)(stop - _start)) / (float)TimeSpan.TicksPerSecond;
                        if ((_relation % 10000) == 0) { GC.Collect(); }
                        OsmSharp.Output.OutputStreamHost.WriteLine("Relation[{0}]: {1}relations/s",
                ...
                case SimpleOsmGeoType.Way:
                    ...
                    if ((_way % 10000) == 0)
                    {
                        ...
                        OsmSharp.Output.OutputStreamHost.WriteLine("Way[{0}]: {1}ways/s @ {2}MB",
                        ...
                        float seconds = ((float)(stop - _start)) / (float)TimeSpan.TicksPerSecond;
                        if ((_way % 100000) == 0) { GC.Collect(); }
                        OsmSharp.Output.OutputStreamHost.WriteLine("Way[{0}]: {1}ways/s",
