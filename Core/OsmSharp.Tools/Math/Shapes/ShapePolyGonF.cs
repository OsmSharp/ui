...
namespace OsmSharp.Tools.Math.Shapes
{
        ...
        where LineType : GenericLineF2D<PointType>
    {
        ...
        public override double Distance(PointType point)
        {
            ...
            for (int idx = 0; idx < _points.Length; idx++)
            {
                ...
                {
                    line = this.PrimitivesFactory.CreateLine(_points[idx], _points[0]);
                }
                else
                {
        ...
        /// <param name="point"></param>
        /// <returns></returns>
        public PolygonProjectionResult<PointType> ProjectOn(PointType point)
        {
            ...
            if (found_line != null)
            {
                PolygonProjectionResult<PointType> result = new PolygonProjectionResult<PointType>();
        ...
        /// <summary>
        /// Return true if any part of this polygon comes inside of the box.
        /// </summary>
        ...
        public override bool Inside(RectangleType box)
        {
            ...
            if (box.IntersectsPotentially(_points[_points.Length - 1], _points[0]))
            {
                if (box.Intersects(_points[_points.Length - 1], _points[0]))
                {
                    return true;
                }
            }
    ...
    where PointType : PointF2D
    {
        internal PolygonProjectionResult()
        {
