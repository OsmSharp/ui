    ...
    public class APIConnection
    {
        ...
        public APICapabilities GetCapabilities()
        {
            ...
            { // get the capabilities from the api.
                ...
                if (response != null && response.Trim().Length > 0)
                {
                    ...
                    Osm.Core.Xml.v0_6.api api = (osm.Osm as OsmSharp.Osm.Core.Xml.v0_6.osm).api;
        ...
        public List<SimpleOsmGeo> BoundingBoxGet(Tools.Math.Geo.GeoCoordinateBox box)
        {
            ...
            if (response != null && response.Trim().Length > 0)
            {
                ...
                OsmSharp.Osm.Core.Xml.v0_6.osm xml_osm = (osm.Osm as OsmSharp.Osm.Core.Xml.v0_6.osm);
                if (xml_osm.node != null)
                {
                    foreach (Osm.Core.Xml.v0_6.node xml_node in xml_osm.node)
                    {
                ...
                if (xml_osm.way != null)
                {
                    foreach (Osm.Core.Xml.v0_6.way xml_way in xml_osm.way)
                    {
                ...
                {
                    foreach (Osm.Core.Xml.v0_6.relation xml_relation in xml_osm.relation)
                    {
        ...
        public SimpleChangeSetInfo ChangeSetGet(long id)
        {
            ...
            if (response != null && response.Trim().Length > 0)
            {
                XmlReaderSource xml_source = new XmlReaderSource(XmlReader.Create(new StringReader(response)));
                OsmDocument osm = new OsmDocument(xml_source);
                Osm.Core.Xml.v0_6.changeset xml_node = (osm.Osm as OsmSharp.Osm.Core.Xml.v0_6.osm).changeset[0];
        ...
        public SimpleChangeSet ChangesGet(long id)
        {
            ...
            if (response != null && response.Trim().Length > 0)
            {
                ...
                Osm.Core.Xml.v0_6.osmChange xml_changeset = serializer.Deserialize(new StringReader(response)) as Osm.Core.Xml.v0_6.osmChange;
        ...
        /// </summary>
        /// <param name="xml_changes"></param>
        /// <returns></returns>
        private SimpleChangeSetInfo Convertv6XmlChangeSet(OsmSharp.Osm.Core.Xml.v0_6.changeset xml_changeset)
        {
            ...
            if (xml_changeset.tag != null)
            {
                ...
                foreach (Osm.Core.Xml.v0_6.tag xml_tag in xml_changeset.tag)
                {
        ...
        /// <returns></returns>
        private SimpleChangeSet Convertv6XmlChanges(OsmSharp.Osm.Core.Xml.v0_6.osmChange osm_change)
        {
            ...
            if (osm_change.create != null)
            {
                ...
                {
                    OsmSharp.Osm.Core.Xml.v0_6.create create = osm_change.create[idx];
        ...
        private SimpleWay Convertv6XmlWay(way xml_way)
        {
            ...
            if (xml_way.nd != null)
            {
                ...
                foreach (Osm.Core.Xml.v0_6.nd xml_nd in xml_way.nd)
                {
        ...
        private SimpleRelation Convertv6XmlRelation(relation xml_relation)
        {
            ...
            if (xml_relation.tag != null)
            {
                ...
                foreach (Osm.Core.Xml.v0_6.tag xml_tag in xml_relation.tag)
                {
            ...
            if (xml_relation.member != null)
            {
                ...
                foreach (Osm.Core.Xml.v0_6.member xml_member in xml_relation.member)
                {
        ...
        public SimpleWay WayCreate(SimpleWay way)
        {
            ...
            // encapsulate into an osm object.
            OsmSharp.Osm.Core.Xml.v0_6.osm osm = new Osm.Core.Xml.v0_6.osm();
            osm.way = new Osm.Core.Xml.v0_6.way[1];
            osm.way[0] = xml_way;
        ...
        public SimpleWay WayGet(long id)
        {
            ...
            if (response != null && response.Trim().Length > 0)
            {
                XmlReaderSource xml_source = new XmlReaderSource(XmlReader.Create(new StringReader(response)));
                OsmDocument osm = new OsmDocument(xml_source);
                Osm.Core.Xml.v0_6.way xml_way = (osm.Osm as OsmSharp.Osm.Core.Xml.v0_6.osm).way[0];
        ...
        public SimpleRelation RelationGet(long id)
        {
            ...
            if (response != null && response.Trim().Length > 0)
            {
                XmlReaderSource xml_source = new XmlReaderSource(XmlReader.Create(new StringReader(response)));
                OsmDocument osm = new OsmDocument(xml_source);
                Osm.Core.Xml.v0_6.relation xml_relation = (osm.Osm as OsmSharp.Osm.Core.Xml.v0_6.osm).relation[0];
        ...
        public SimpleRelation RelationCreate(SimpleRelation relation)
        {
            ...
            // encapsulate into an osm object.
            OsmSharp.Osm.Core.Xml.v0_6.osm osm = new Osm.Core.Xml.v0_6.osm();
            osm.relation = new Osm.Core.Xml.v0_6.relation[1];
