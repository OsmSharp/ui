                ...
                if (_reader.NodeType == XmlNodeType.Element && (_reader.Name == "node" || _reader.Name == "way" || _reader.Name == "relation"))
                {
                    ...
                    switch (name)
                    {
                         case "node":
                             ...
                             if (osm_obj is OsmSharp.Osm.Xml.v0_6.node)
                             {
                                 _next = XmlSimpleConverter.ConvertToSimple(osm_obj as OsmSharp.Osm.Xml.v0_6.node);
                                 return true;
                             }
                             break;
                         case "way":
                             osm_obj = _ser_way.Deserialize(reader);
                             if (osm_obj is OsmSharp.Osm.Xml.v0_6.way)
                             {
                                 _next = XmlSimpleConverter.ConvertToSimple(osm_obj as OsmSharp.Osm.Xml.v0_6.way);
                                 return true;
                             }
                             break;
                         case "relation":
                             osm_obj = _ser_relation.Deserialize(reader);
                             if (osm_obj is OsmSharp.Osm.Xml.v0_6.relation)
                             {
                                 _next = XmlSimpleConverter.ConvertToSimple(osm_obj as OsmSharp.Osm.Xml.v0_6.relation);
                                 return true;
                             }
                             break;
                    }
