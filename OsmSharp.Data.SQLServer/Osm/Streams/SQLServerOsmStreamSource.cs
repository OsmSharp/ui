    ...
    public class SQLServerOsmStreamSource : OsmStreamSource
    {
            ...
            if (!_relationReader.IsClosed)
            {
                ...
                if (!_relationMemberReader.IsClosed)
                {
                    ...
                    while (returnedId == relation.Id.Value)
                    {
                        ...
                        int memberType = _relationMemberReader.GetInt32(1);
                        ...
                        member.MemberType = (OsmGeoType) memberType;
        ...
        private bool DoMoveNextNode()
        {
        	...
        	if (!_nodeReader.IsClosed)
        	{
        		...
        		node.Latitude = _nodeReader.GetInt32(1)/10000000.0;
                node.Longitude = _nodeReader.GetInt32(2) / 10000000.0;
                node.ChangeSetId = _nodeReader.IsDBNull(3) ? null : (long?)_nodeReader.GetInt64(3);
                node.TimeStamp = _nodeReader.IsDBNull(5) ? null : (DateTime?)_nodeReader.GetDateTime(5);
                node.Version = _nodeReader.IsDBNull(7) ? null : (ulong?)(ulong)_nodeReader.GetInt32(7);
                node.Visible = _nodeReader.IsDBNull(4) ? null : (bool?)(_nodeReader.GetValue(4));
    	...
    	private bool DoMoveNextWay()
        {
			if (_wayReader == null)
			{
                ...
                SqlCommand way_tag_command = new SqlCommand("select * from way_tags order by way_id");
            ...
            if (!_wayReader.IsClosed)
            {
				...
				way.Version = (ulong)_wayReader.GetInt32(4);
				way.Visible = _wayReader.GetBoolean(3);
