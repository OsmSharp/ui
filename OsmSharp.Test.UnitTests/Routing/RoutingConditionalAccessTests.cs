...
namespace OsmSharp.Test.Unittests.Routing
{
        ...
        where EdgeData : IDynamicGraphEdgeData
    {
        ...
        protected void DoTestShortestWithAccess(Vehicle vehicle, List<KeyValuePair<string, string>> accessTags)
        {
            ...
            Assert.AreEqual(RouteSegmentType.Stop, route.Entries[3].Type);
        }
        ...
        protected void DoTestShortestWithoutAccess(Vehicle vehicle, List<KeyValuePair<string, string>> access_tags)
        {
            ...
            Assert.AreEqual(RouteSegmentType.Along, route.Entries[4].Type);

            data.GetVertex(19, out latitude, out longitude);
            Assert.AreEqual(latitude, route.Entries[5].Latitude, 0.00001);
            Assert.AreEqual(longitude, route.Entries[5].Longitude, 0.00001);
            Assert.AreEqual(RouteSegmentType.Stop, route.Entries[5].Type);
        }
