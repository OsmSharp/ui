...
namespace OsmSharp.Data.PostgreSQL.Osm
{
    ...
    public class PostgreSQLSnapshotSource : IDataSourceReadOnly, IDisposable
    {
        ...
        public PostgreSQLSnapshotSource(string connectionString)
        {
        ...
        private void ParseTags(string tags, TagsCollection target)
        {
            if (tags.Length > 0)
            {
        ...
        public string Name
        {
            get 
            { 
                return "PostgreSQL Snapshot Schema Data Source"; 
            }
        }
        ...
        public Way GetWay(long id)
        {
            ...
            while (reader.Read())
            {
                ...
                node = new Node();
                node.Id = node_id;
                node.Version = (ulong)node_version;
        ...
        public IList<OsmGeo> Get(GeoCoordinateBox box, OsmSharp.Osm.Filters.Filter filter)
        {
            ...
            if (filter != null && filter != OsmSharp.Osm.Filters.Filter.Any())
            {
